...
function displayResults(results) {
    ...
    const resultsHTML = results.map(result => `
        <div class="result-card">
            ...
            <p>To'g'ri javoblar: ${result.correctAnswers}/${result.totalQuestions}</p>
           
            <p>Ball: ${result.score}%</p>
            <p>Sarflangan vaqt: ${Math.floor(result.timeSpent / 60)}:${(result.timeSpent % 60).toString().padStart(2, '0')}</p>
        </div>
    `).join('');
...
function displayAchievements(results) {
    ...
    const achievementsHTML = achievements.map(achievement => `
        <div class="achievement-card ${achievement.unlocked ? 'unlocked' : 'locked'}">
            ...
            ${achievement.unlocked ? `<p class="unlock-date">Ochilgan sana: ${new Date(achievement.unlockDate).toLocaleDateString()}</p>` : ''}
        </div>
    `).join('');
    ...
    btn.addEventListener('click', () => {
        ...
        resultCards.forEach(card => {
            ...
            if (category === 'barchasi') {
                ...
                card.style.display = testName.includes('javascript') || 
                                   testName.includes('python') || 
                                   testName.includes('dasturlash') ? 'block' : 'none';
            } else if (category === 'mathematics') {
                card.style.display = testName.includes('algebra') || 
                                   testName.includes('geometriya') || 
                                   testName.includes('matematika') ? 'block' : 'none';
            } else if (category === 'logic') {
                card.style.display = testName.includes('mantiq') || 
                                   testName.includes('logic') ? 'block' : 'none';
            }
        });
    });
});
